...
class UserManager(models.Manager):
    def validate_reg(self, input):      # def validate_reg(self, *kwargs):
        errors = []
        fname = input['fname']
        lname = input['lname']
        email = input['email']
        ...
        if not dob:
            errors.append(("Please select your birthday!","dob"))
        else:
            time_delta = (datetime.now() - datetime.strptime(dob, '%Y-%m-%d')).days
            if time_delta <= 0:
                errors.append(("Birthday should before today!","dob"))
            elif time_delta < 6574:
                errors.append(("You should be at least 18 years old!","dob"))
